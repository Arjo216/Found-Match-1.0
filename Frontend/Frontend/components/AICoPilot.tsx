...
import { Bot, Send, X, Loader2, Sparkles, BrainCircuit, Activity, ShieldCheck, Paperclip, FileText, Maximize2, Minimize2, Printer } from "lucide-react";
...
export default function AICoPilot({ profileId }: { profileId: number }) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "I am your Executive Co-Pilot. I can analyze market trends, review attached pitch decks, and help you draft institutional materials. How can I assist you today?", timestamp: new Date().toISOString() }
  ]);
  ...
  useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages, isTyping, suggestions]);

  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (file) setAttachedFile(file); };
  ...
  const handleSend = async (e?: React.FormEvent, customText?: string) => {
    ...
    try {
      ...
      if (fileToSend) {
        ...
        const res = await api.post("/agent/analyze-document", formData, { headers: { "Content-Type": "multipart/form-data" } });
        replyText = res.data.reply;
      } else {
        const res = await api.post("/agent/copilot", { profile_id: profileId, message: userMsg, history: messages.slice(-4) });
        replyText = res.data.reply;
      }
      setMessages(prev => [...prev, { role: "assistant", content: replyText, timestamp: new Date().toISOString() }]);
    } catch (err) {
      setMessages(prev => [...prev, { role: "assistant", content: "System error. Please reconnect.", timestamp: new Date().toISOString() }]);
  ...
  const handleExportPDF = () => {
    ...
    const html = `<html><head><title>AI Strategy Dossier</title><style>body{font-family:sans-serif;padding:40px;line-height:1.6}.message{margin-bottom:20px;padding:15px;background:#f9f9f9;border-left:4px solid #ddd}.message.ai{border-left-color:#7c3aed;background:#faf5ff}.msg-header{font-size:11px;color:#888;margin-bottom:8px;font-weight:bold}</style></head><body><h2>AI Co-Pilot Strategy Dossier</h2>${messages.map(m => `<div class="message ${m.role === 'assistant' ? 'ai' : ''}"><div class="msg-header">${m.role === 'assistant' ? 'FoundMatch AI' : 'You'}</div><div>${m.content}</div></div>`).join('')}<script>setTimeout(()=>window.print(),500);</script></body></html>`;
  ...
  return (
    <>
      <AnimatePresence>
        {!isOpen && (
          <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} onClick={() => setIsOpen(true)} style={{ position: 'fixed', bottom: '32px', right: '32px', zIndex: 99990 }} className="w-16 h-16 bg-gradient-to-tr from-cyan-600 to-purple-600 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(8,145,178,0.4)] hover:shadow-[0_0_40px_rgba(168,85,247,0.6)] transition-all group border-2 border-white/30">
            ...
            <motion.div layout initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0, width: isMaximized ? "100vw" : "100%", maxWidth: isMaximized ? "100vw" : "800px", height: isMaximized ? "100vh" : "85vh", maxHeight: isMaximized ? "100vh" : "900px", borderRadius: isMaximized ? "0px" : "16px" }} exit={{ opacity: 0, scale: 0.95, y: 20 }} transition={{ duration: 0.3 }} style={{ position: "relative", display: "flex", flexDirection: "column", backgroundColor: "rgba(2, 6, 23, 0.95)", border: isMaximized ? "none" : "1px solid rgba(168, 85, 247, 0.4)", boxShadow: "0 30px 80px rgba(0,0,0,0.9)" }} className="overflow-hidden">
              ...
              <div style={{ flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px", borderBottom: "1px solid rgba(255,255,255,0.1)", backgroundColor: "rgba(15, 23, 42, 1)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                  <div style={{ width: "48px", height: "48px", borderRadius: "50%", backgroundColor: "rgba(168, 85, 247, 0.1)", border: "1px solid rgba(168, 85, 247, 0.3)", display: "flex", alignItems: "center", justifyContent: "center", color: "#c084fc" }}><BrainCircuit size={24} /></div>
                  <div>
                    <h3 style={{ color: "white", fontWeight: "bold", fontSize: "18px", display: "flex", alignItems: "center", gap: "8px", margin: 0 }}>Executive Co-Pilot<span style={{ fontSize: "10px", backgroundColor: "rgba(16, 185, 129, 0.1)", color: "#34d399", padding: "2px 8px", borderRadius: "9999px", border: "1px solid rgba(16, 185, 129, 0.2)", display: "flex", alignItems: "center", gap: "4px" }}><Activity size={10} /> ONLINE</span></h3>
                  </div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                  <button onClick={handleExportPDF} style={{ background: "transparent", border: "none", color: "#94a3b8", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }} className="hover:text-purple-400 transition-colors" title="Export Offline AI Dossier"><Printer size={20} /></button>
                  ...
                  <button onClick={() => setIsOpen(false)} style={{ background: "transparent", border: "none", color: "#94a3b8", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }} className="hover:text-rose-400 transition-colors" title="Close"><X size={24} /></button>
                </div>
              </div>
                  ...
                  {messages.map((msg, idx) => {
                    ...
                    return (
                      <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ display: "flex", flexDirection: "column", alignItems: isUser ? "flex-end" : "flex-start", marginBottom: "20px" }}>
                        <div style={{ maxWidth: "80%", padding: "16px 20px", fontSize: "15px", lineHeight: "1.7", borderRadius: "16px", borderTopRightRadius: isUser ? "4px" : "16px", borderTopLeftRadius: !isUser ? "4px" : "16px", backgroundColor: isUser ? "#4c1d95" : "#1e293b", color: isUser ? "white" : "#e2e8f0", border: `1px solid ${isUser ? "#7c3aed" : "#475569"}`, whiteSpace: "pre-wrap" }}>
              ...
              {/* --- NEW: FILE ATTACHMENT INDICATOR POPUP --- */}
              <AnimatePresence>
                {attachedFile && (
              ...
              <AnimatePresence>
                {suggestions.length > 0 && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} style={{ padding: "16px 24px", backgroundColor: "#0a0f1c", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", flexDirection: "column", gap: "12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px", fontWeight: "bold", color: "#c084fc" }}><Sparkles size={16} /> Recommended Queries <button onClick={() => setSuggestions([])} style={{ marginLeft: "auto", background: "transparent", border: "none", color: "#64748b", cursor: "pointer" }}><X size={16} /></button></div>
                    {suggestions.map((suggestion, idx) => (
                      <button key={idx} onClick={() => { setInput(suggestion); setSuggestions([]); }} style={{ textAlign: "left", fontSize: "14px", color: "#f3e8ff", backgroundColor: "rgba(88, 28, 135, 0.4)", border: "1px solid rgba(168, 85, 247, 0.6)", padding: "14px", borderRadius: "12px", cursor: "pointer" }}>"{suggestion}"</button>
                    ))}
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <form onSubmit={handleSend} style={{ flexShrink: 0, padding: "20px 24px", borderTop: "1px solid rgba(255,255,255,0.1)", backgroundColor: "rgba(15, 23, 42, 1)", display: "flex", gap: "12px", alignItems: "center" }}>
                ...
                <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isTyping} title="Upload Document for AI Review" style={{ width: "48px", height: "48px", flexShrink: 0, borderRadius: "50%", backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", color: "#94a3b8", cursor: isTyping ? "not-allowed" : "pointer" }}><Paperclip size={20} /></button>
                <button type="button" onClick={fetchAISuggestions} disabled={isGenerating || isTyping} style={{ width: "48px", height: "48px", flexShrink: 0, borderRadius: "50%", backgroundColor: "rgba(147, 51, 234, 0.2)", border: "1px solid rgba(168, 85, 247, 0.5)", display: "flex", alignItems: "center", justifyContent: "center", color: "#c084fc", cursor: isGenerating || isTyping ? "not-allowed" : "pointer" }}>{isGenerating ? <Loader2 size={24} className="animate-spin" /> : <BrainCircuit size={24} />}</button>
